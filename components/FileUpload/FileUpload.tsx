...
const FileUpload = () => {
    ...
    const onDrop = useCallback(
        (acceptedFiles: File[]) => {
            ...
            if (!parsed.success) {
                toast.error(parsed.error.errors.map((err) => err.message).join('\n'));
                return;
            }
    ...
    return (
      ...
      `}
        ...
        >
            ...
            {/* 배경 그라데이션 효과 수정 */}
            <div
                className="absolute scale-80 -inset-1 rounded-xl bg-gradient-to-br from-orange-50 to-yellow-50 dark:from-orange-300 dark:to-yellow-950 opacity-50"/>

            {/* 아이콘 컨테이너 */}
            <div className="relative">
                <div
                    className="absolute inset-0 w-16 h-16 bg-orange-500/10 dark:bg-orange-500/20 rounded-full blur-xl transform group-hover:scale-125 transition-transform duration-300"/>
                <ImagePlus
                    className="w-32 h-32 text-orange-500 dark:text-orange-400 relative transform group-hover:scale-110 transition-transform duration-300"/>
            </div>
