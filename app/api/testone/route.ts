...
export async function POST(request: NextRequest) {
    try {
        ...
        backendFormData.append('taskId', validatedData.taskId);
        backendFormData.append(optionParam, validatedData.options);
        ...
        // 콘솔 로그: 파일 제외한 FormData의 필드 값 출력
        console.log('FormData 필드 (파일 제외):');
        ...
        const response = await fetch(
            `${process.env.SPRING_BACKEND_URL}/${validatedData.method}`,
            {
                method: 'POST',
